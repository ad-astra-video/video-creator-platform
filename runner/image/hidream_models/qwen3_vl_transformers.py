...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3VLTextConfig, device=None):
        ...
        self.config = config
        # Compat: the HiDream-O1 config declares rope_type "default", which
        # transformers renamed to "linear" (>=5.15). Normalize so the lookup
        # works against the installed transformers' ROPE_INIT_FUNCTIONS.
        _rope_key = self.rope_type if self.rope_type in ROPE_INIT_FUNCTIONS else "linear"
        self.rope_init_fn = ROPE_INIT_FUNCTIONS[_rope_key]
        # The 5.15 linear rope init reads rope_scaling["factor"]; the
        # HiDream-O1 config omits it (old "default" implied factor 1.0).
        _rs = config.rope_scaling
        if isinstance(_rs, dict) and "factor" not in _rs:
            _rs = {**_rs, "factor": 1.0}
            config.rope_scaling = _rs

        inv_freq, self.attention_scaling = self.rope_init_fn(self.config, device)
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        # Compat: register original_inv_freq as a real buffer too. Under
        # transformers 5.15 lazy loading the plain Python alias
        # `self.original_inv_freq = self.inv_freq` ended up a meta tensor
        # (the underlying inv_freq buffer is replaced in place), so a second
        # registered buffer keeps a materialized copy that survives .to(cuda).
        self.register_buffer("original_inv_freq", inv_freq.clone(), persistent=False)

        self.mrope_section = config.rope_scaling.get("mrope_section", [24, 20, 20])

    # Compat: transformers >=5.15 calls this on RotaryEmbedding during weight
    # initialization when rope_type == "default". The vendored 4.57-era class
    # never defined it; delegate to the same (normalized) linear/factor-1.0
    # init used above so inv_freq is consistent.
    def compute_default_rope_parameters(self, config=None):
        ...
        return self.rope_init_fn(config)
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Compat: transformers >=5.15 drop the `cache_position` arg (marked
        # deprecated+unused) and use `inputs_embeds` (not `input_embeds`).
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def _forward_generation(self, input_ids, position_ids, vinputs, timestep, token_types,
        ...
        # 4. Embed vinputs and append to sequence
