...
import os
# Local edit vs upstream: flash-attn is NOT in the image (its arch-matched build
# is too heavy to compile at image-build time), so this hardcodes the eager
# 4D attention-mask fallback. Flip to True only if you ship a prebuilt
# flash-attn wheel matching the host CUDA arch — there is no runtime env toggle.
