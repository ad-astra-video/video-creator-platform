...
interface VideoPreviewPanelProps {
  ...
  errorMessage?: string
  // When false, skip building the filmstrip thumbnail carousel (no frame extraction,
  // none rendered). RestylePanel only needs the first frame (fetched separately via
  // /restyle/extract-first-frame), so it sets this to avoid pulling every frame.
  showFilmstrip?: boolean
  // Programmatic source-load request (e.g. a quick-pick asset tile click). When the
  // caller bumps `token` with a non-empty `path`, the panel swaps to that source
  // (same behavior as drop/browse). Pass null to leave untouched.
  externalVideoRequest?: { path: string; token: number } | null
...
export function VideoPreviewPanel({
  ...
  useEffect(() => {
    if (!videoUrl || extractingRef.current || videoDuration <= 0 || !showFilmstrip) return
  ...
  }, [videoUrl, videoDuration, thumbCount, showFilmstrip])
  ...
  // Load a programmatically-requested source (quick-pick tile) when its token changes.
  useEffect(() => {
    if (!externalVideoRequest?.path) return
    selectSource(externalVideoRequest.path)
  }, [externalVideoRequest, selectSource])
