...
interface LtxApiKeyInputProps {
  ...
  masked?: boolean
  /** Disable the input (used to show a read-only configured value). */
  disabled?: boolean
}

export const LtxApiKeyInput = forwardRef<HTMLInputElement, LtxApiKeyInputProps>(
  ({ value, onChange, placeholder = 'Paste your API key', id, stopPropagation, className, masked = true, disabled = false }, ref) => {
    return (
      <div className={`relative ${className ?? ''}`}>
        ...
        <input
          ...
          autoComplete="off"
          disabled={disabled}
          className="w-full rounded-lg border border-zinc-700 bg-zinc-900 py-2.5 pl-9 pr-3 text-sm text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-70 disabled:bg-zinc-950"
        />
      </div>
    )
  },
