// Fixed bottom-right generation task list + completion/error toasts.
// Reads the global GenerationTaskProvider (mounted above routing), so the list survives
// navigation and shows every background generation the user has fired.

import { useEffect, useState } from 'react'
import {
  AlertCircle, CheckCircle2, ChevronDown, ListTodo,
  Loader2, X, XCircle,
} from 'lucide-react'
...
export function GenerationTaskPanel() {
  const { tasks, toasts, runningCount: running, clearCompleted, clearTask, dismissToast } = useGenerationTasks()
  useNow()

  if (tasks.length === 0 && toasts.length === 0) return null

  return (
    <>
      {/* transient toasts — always visible above the panel */}
      {toasts.length > 0 && (
        <div className="pointer-events-none fixed bottom-24 right-4 z-[90] flex w-80 flex-col gap-2">
          {toasts.map(t => (
            <div
              key={t.id}
              className={`pointer-events-auto flex items-start gap-2 rounded-lg border px-3 py-2 text-sm shadow-xl backdrop-blur ${
                t.tone === 'error'
            ...
            >
              {t.tone === 'error' ? <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /> : <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" />}
              <span className="min-w-0 flex-1 break-words">{t.message}</span>
              <button onClick={() => dismissToast(t.id)} className="shrink-0 rounded p-0.5 text-zinc-400 hover:bg-white/10 hover:text-zinc-100" aria-label="Dismiss">
        ...
        <div className="overflow-hidden rounded-xl border border-zinc-700/80 bg-zinc-900/95 shadow-2xl backdrop-blur">
          ...
          <div className="border-t border-zinc-800">
            <div className="max-h-72 overflow-y-auto">
              {tasks.length === 0 ? (
                <div className="px-3 py-4 text-center text-xs text-zinc-500">No generations yet.</div>
              ) : (
                tasks.map(t => (
