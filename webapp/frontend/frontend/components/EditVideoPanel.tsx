...
import {
  useBerniniEdit,
  type BerniniEditOutcome,
} from "../hooks/use-bernini-edit";
...
export const EditVideoPanel = forwardRef<
  EditVideoPanelHandle,
  EditVideoPanelProps
>(function EditVideoPanel(
  ...
  const applyEngine = useCallback((m: BerniniEngine) => {
    setEng(m);
    if (m === "14b")
      setReferencePaths((prev) => (prev.length > 1 ? prev.slice(0, 1) : prev));
  }, []);
  useEffect(() => {
    applyEngine(engine);
  }, [engine, applyEngine]);
  ...
  // 14B-only — the backend loads the LoRA only on the fp8 14B renderer. The toggle
  // is disabled when the Fast (1.3B) engine is selected.
  ...
  const handleSourceChange = useCallback(
    (data: { videoPath: string | null }) => {
      setVideoPath(data.videoPath);
      onSourceChangeRef.current?.({
        videoPath: data.videoPath,
        ready: !!data.videoPath,
      });
    },
    [],
  );
  ...
  useImperativeHandle(
    ...
    () => ({
      runEdit: async (prompt, opts) => {
        ...
        const fps =
          post.fps_boost?.target_fps ??
          (sourceFps && sourceFps > BERNINI_NATIVE_FPS
            ? sourceFps
            : BERNINI_NATIVE_FPS);
        const resolution = post.upscale
          ? resolutionForPost(post)
          : BERNINI_NATIVE_RESOLUTION;
        return submitBerniniEdit({
          ...
          resolution,
          turbo,
          // Full-quality non-turbo render = 40 denoise steps (threaded explicitly
          // so the runner doesn't fall back to its 20-step default). Turbo path
          // leaves it unset -> renderer uses its own 4-step LoRA recipe.
          numInferenceSteps: turbo ? undefined : 40,
          duration: 3,
    ...
    <div className="flex flex-col gap-3 h-full min-h-0">
      ...
      <div className="flex-1 min-h-0">
        <VideoPreviewPanel
          ...
          hint={{
            title: "Pick an edit goal, then describe the edit in the bar below",
          }}
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] font-medium transition-colors ${
            turbo && eng === "14b"
              ...
              : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700 disabled:opacity-40 disabled:hover:bg-zinc-800"
          }`}
          title={
            eng === "1.3b"
              ? "4-step LightX2V distill is 14B-only — switch to Detailed (14B)"
              : "Turbo: 4-step LightX2V distill instead of 40-step render"
          }
        >
      ...
      {goal === "r2v" && (
        ...
        >
          ...
          {referencePaths.map((p, i) => (
            <span
              key={p}
              className="relative group flex items-center gap-1 text-[10px] text-zinc-300 bg-zinc-800 rounded-md px-1 py-0.5 pr-5"
            >
              <img
